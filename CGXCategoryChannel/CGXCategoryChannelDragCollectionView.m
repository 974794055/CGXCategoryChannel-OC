//  CGXCategoryChannelDragCollectionView.m
//  CGXMenu
//
...
#pragma mark - UICollectionView (CGXCategoryChannelDragCollectionViewRect)

/**
 内部工具
 */
@interface UICollectionView (CGXCategoryChannelDragCollectionViewRect)
...
/**
 ...
 @return Rect
 */
- (CGRect)CGXCategoryChannelDragCollectionView_rectForSection:(NSInteger)section;
...
/**
 ...
 @return Rect
 */
- (CGRect)CGXCategoryChannelDragCollectionView_rectForRowAtIndexPath:(NSIndexPath *)indexPath;
...
- (CGRect)CGXCategoryChannelDragCollectionView_rectForSection:(NSInteger)section {
    ...
    if (sectionNum <= 0) {
        ...
        CGRect firstRect = [self CGXCategoryChannelDragCollectionView_rectForRowAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
        CGRect lastRect = [self CGXCategoryChannelDragCollectionView_rectForRowAtIndexPath:[NSIndexPath indexPathForItem:sectionNum-1 inSection:section]];
...
typedef NS_ENUM(NSUInteger, CGXCategoryChannelDragCollectionViewScrollDirection) {
    CGXCategoryChannelDragCollectionViewScrollDirectionNone = 0,
    CGXCategoryChannelDragCollectionViewScrollDirectionLeft,
    CGXCategoryChannelDragCollectionViewScrollDirectionRight,
    CGXCategoryChannelDragCollectionViewScrollDirectionUp,
    CGXCategoryChannelDragCollectionViewScrollDirectionDown,
};

@interface CGXCategoryChannelDragCollectionView ()
...
@implementation CGXCategoryChannelDragCollectionView
...
// 处理临界点问题
- (CGXCategoryChannelDragCollectionViewScrollDirection)_setScrollDirection {
    if (self.bounds.size.height + self.contentOffset.y - _snapedView.center.y < _snapedView.bounds.size.height / 2 && self.bounds.size.height + self.contentOffset.y < self.contentSize.height) {
        return CGXCategoryChannelDragCollectionViewScrollDirectionDown;
    }
    if (_snapedView.center.y - self.contentOffset.y < _snapedView.bounds.size.height / 2 && self.contentOffset.y > 0) {
        return CGXCategoryChannelDragCollectionViewScrollDirectionUp;
    }
    if (self.bounds.size.width + self.contentOffset.x - _snapedView.center.x < _snapedView.bounds.size.width / 2 && self.bounds.size.width + self.contentOffset.x < self.contentSize.width) {
        return CGXCategoryChannelDragCollectionViewScrollDirectionRight;
    }
    if (_snapedView.center.x - self.contentOffset.x < _snapedView.bounds.size.width / 2 && self.contentOffset.x > 0) {
        return CGXCategoryChannelDragCollectionViewScrollDirectionLeft;
    }
    return CGXCategoryChannelDragCollectionViewScrollDirectionNone;
}
...
- (void)_edgeScroll {
    CGXCategoryChannelDragCollectionViewScrollDirection scrollDirection = [self _setScrollDirection];
    switch (scrollDirection) {
        case CGXCategoryChannelDragCollectionViewScrollDirectionLeft:{
            //这里的动画必须设为NO
            ...
            break;
        case CGXCategoryChannelDragCollectionViewScrollDirectionRight:{
            [self setContentOffset:CGPointMake(self.contentOffset.x + 4, self.contentOffset.y) animated:NO];
            _snapedView.center = CGPointMake(_snapedView.center.x + 4, _snapedView.center.y);
            _lastPoint.x += 4;
        }
            break;
        case CGXCategoryChannelDragCollectionViewScrollDirectionUp:{
            ...
            break;
        case CGXCategoryChannelDragCollectionViewScrollDirectionDown:{
            [self setContentOffset:CGPointMake(self.contentOffset.x, self.contentOffset.y + 4) animated:NO];
            _snapedView.center = CGPointMake(_snapedView.center.x, _snapedView.center.y + 4);
            _lastPoint.y += 4;
        }
    ...
    if (scrollDirection == CGXCategoryChannelDragCollectionViewScrollDirectionNone) {
        return;
    }
    __weak typeof(self) weakSelf = self;
    // 如果Cell 拖拽到了边沿时
    ...
    [UIView animateWithDuration:0.1 animations:^{
        weakSelf.snapedView.center = weakSelf.lastPoint;
    }];
    ...
    switch (longGesture.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            [UIView animateWithDuration:0.25 animations:^{
                weakSelf.snapedView.transform = CGAffineTransformMakeScale(weakSelf.dragZoomScale, weakSelf.dragZoomScale);
                weakSelf.snapedView.center = CGPointMake(currentPoint.x, currentPoint.y);
                weakSelf.snapedView.alpha = weakSelf.dragCellAlpha;
            }];
        ...
        default: {
            ...
            if (self.delegate
                ...
                for (NSInteger i = 0; i < sec; i++) {
                    if (CGRectContainsPoint([self CGXCategoryChannelDragCollectionView_rectForSection:i], point)) {
                        section = i;
                        break;
                    }
                }
            ...
            [UIView animateWithDuration:0.25 animations:^{
                if (!cell) {
                    weakSelf.snapedView.center = weakSelf.oldPoint;
                } else {
                    weakSelf.snapedView.center = cell.center;
                }
                weakSelf.snapedView.transform = CGAffineTransformMakeScale(1.0f, 1.0f);
                weakSelf.snapedView.alpha = 1.0;
            } completion:^(BOOL finished) {
                //移除截图视图、显示隐藏的cell并开启交互
                [weakSelf.snapedView removeFromSuperview];
                cell.hidden = NO;
                self.userInteractionEnabled = YES;
